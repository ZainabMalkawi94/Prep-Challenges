...
const objLat = (obj) => {
    const cptlFName = obj.firstName.charAt(0).toUpperCase() + obj.firstName.slice(1);
    const cptlLName = obj.lastName.charAt(0).toUpperCase() + obj.lastName.slice(1);
    let fName = cptlFName + " " + cptlLName;
    return `my name is ${fName} I am ${obj.age} YO, and I love ${obj.hobby}.`
};
// -------------------------------------------------------------------------------------------------------
    ...
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].yearsOfExperience > 1) {
            if (arr[i].firstName != null) {
                fName = `${arr[i].firstName}`;
            }
            if (arr[i].lastName != null) {
                fName += " " + arr[i].lastName;
            }
            filteredArr.push({
                "fullName": fName,
                "tech": arr[i].tech
